...
func RunChronos(
	...
	RunLoopWithOptions(
		...
		RunOptions{
			...
			CompactionContext: cloneCompactionContext(cfg.CompactionContext),
			Recorder:          cfg.Recorder,
			HarnessProfile:    cfg.HarnessProfile,
			CapabilityProfile: cfg.CapabilityProfile,
			PlanAnchor:        anchor,
			PlanBinding:       cfg.PlanBinding,
			...
			SkillDirs:         cloneStringsPreserveNil(cfg.SkillDirs),
			ProjectSkillDirs:  cloneStringsPreserveNil(cfg.ProjectSkillDirs),
			RunMode:           mode,
