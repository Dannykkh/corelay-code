...
Use RepoMap for a structural overview, LSP for semantic Go navigation when available, Glob to find files, Grep to search content, and Read to read files.
Do NOT modify any files. Report findings concisely.`,
			Tools:    []string{"Read", "Glob", "Grep", "RepoMap", "LSP", "Bash", "LS"},
			ReadOnly: true,
		},
		"researcher": {
			Name:        "researcher",
...
Do NOT modify any files.`,
			Tools:    []string{"Read", "Glob", "Grep", "RepoMap", "LSP", "Bash"},
			ReadOnly: true,
		},
		"tester": {
			Name:        "tester",
