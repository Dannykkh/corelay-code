...
func TestApplyToolResponsePolicyGolden(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "multi-format accepts Continue tool codeblock",
			policy:      harness.ResponseMultiFormat,
			visible:     "```tool\nTOOL_NAME: Read\nBEGIN_ARG: file_path\n\"notes.txt\"\nEND_ARG\n```",
			wantStatus:  ToolParseParsed,
		...
		{
			name:        "multi-format accepts goose direct token markers",
			policy:      harness.ResponseMultiFormat,
			visible:     `<|tool_calls_section_begin|><|tool_call_begin|>functions.Read:0<|tool_call_argument_begin|>{"file_path":"notes.txt"}<|tool_call_end|><|tool_calls_section_end|>`,
			wantStatus:  ToolParseParsed,
